...
test-vectors/      Language-independent fixtures. Authoritative for canonical
                   JSON, entry hashing, and Ed25519 signatures.
...
- The test vectors in `test-vectors/vectors.json` are
  authoritative. Changing a value changes the contract every
  implementation must follow, so it is never a casual edit.
...
## Test vectors are sacred

`test-vectors/vectors.json` is generated by the Rust implementation
(`cargo run --example gen_fixture`) and re-asserted by
`spine-core/tests/cross_language_vectors.rs`. The values are a wire
contract: they double as the spec any future independent
implementation must reproduce byte-for-byte. Do not change a vector
unless you have:

1. Identified what the vector currently asserts and *why*: read
   the existing case's `description` and the surrounding context.
2. Confirmed the new value is correct in its own right (not merely
   what the current code happens to emit) and regenerated the file.
3. Bumped the relevant `_version` field in `vectors.json` and
   noted the change in the PR description.

A "fix" that just changes the expected value to match new code
output is a regression in disguise: the vector exists to catch
exactly that.
